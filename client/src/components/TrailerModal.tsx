...
export default function TrailerModal({ movieData, onClose }: TrailerModalProps) {
  return (
    <div 
      className="fixed inset-0 z-50 flex items-center justify-center trailer-modal"
      onClick={onClose}
      data-testid="trailer-modal-overlay">
      <div className="relative max-w-4xl w-full mx-4" onClick={(e) => e.stopPropagation()}>
        <button 
          onClick={onClose} 
          className="absolute -top-12 right-0 text-white hover:text-gray-300 text-2xl transition-colors"
        ...
        <div className="bg-card rounded-lg overflow-hidden shadow-2xl">
          ...
          <div className="aspect-video bg-black flex items-center justify-center relative">
            ...
            <div className="relative z-10 text-center">
              <button 
                className="bg-accent/20 backdrop-blur-sm border-2 border-accent rounded-full w-20 h-20 flex items-center justify-center hover:bg-accent/30 transition-all duration-300 mb-4"
